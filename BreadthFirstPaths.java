...
public class BreadthFirstPaths {
    ...
    private boolean check(EdgeWeightedGraph G, int s) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (Edge w : G.adj(v)) {
                if (hasPathTo(v) != hasPathTo(w.other(v))) {
                	System.out.println("edge " + v + "-" + w.other(v));
                	System.out.println("hasPathTo(" + v + ") = " + hasPathTo(v));
                	System.out.println("hasPathTo(" + w.other(v) + ") = " + hasPathTo(w.other(v)));
                    return false;
                }
                if (hasPathTo(v) && (distTo[w.other(v)] > distTo[v] + 1)) {
                	System.out.println("edge " + v + "-" + w.other(v));
                	System.out.println("distTo[" + v + "] = " + distTo[v]);
                	System.out.println("distTo[" + w.other(v) + "] = " + distTo[w.other(v)]);
                    return false;
                }
            }
        }
