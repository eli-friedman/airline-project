    	...
    	try {
    	  //  int E = Integer.parseInt(br.readLine());
    	    ...
    	    while (line != null) {
    	    	...
    	    	int v = Integer.parseInt(lineArr[0]) - 1;
    	    	String vCity = cities[v];
	            int w = Integer.parseInt(lineArr[1]) - 1;
	            String wCity = cities[w];
	            int distance = Integer.parseInt(lineArr[2]);
	            double price = Double.parseDouble(lineArr[3]);
	            Edge e = new Edge(v,vCity, w, wCity, distance, price);
	            addEdge(e);
	            line = br.readLine();
...
*/
    ...
    public String showAllRoutes() {
        ...
        for (int v = 0; v < V; v++) {
            for (Edge e : adj[v]) {
            	if(e.other(v) > v){
            		s.append(e.toStringWithReverse() + "  ");
            		s.append(NEWLINE);
            	}
            }
        }
    ...
    public String getCityName(int c) {
    	for (int v = 0; v < V; v++) {
            for (Edge e : adj[v]) {
            	if(e.v() == c ){
            		return e.vCity();
